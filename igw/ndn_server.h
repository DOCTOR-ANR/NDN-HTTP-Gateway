...
private:
    ...
    public:
        ...
        std::chrono::steady_clock::time_point get_last_access();

        void add_data(uint64_t segment, std::shared_ptr<ndn::Data> data);

        std::shared_ptr<ndn::Data> get_data(uint64_t segment);
    };
...
public:
    ...
    void resolve_handler(std::shared_ptr<NdnMessage> ndn_message);

    void on_interest(const ndn::Interest &interest);

    void on_interest_handler(ndn::Interest interest, std::shared_ptr<boost::asio::deadline_timer> timer, int left);

    void generate_data(std::shared_ptr<NdnMessage> ndn_message, std::shared_ptr<boost::asio::deadline_timer> timer, uint64_t segment);
