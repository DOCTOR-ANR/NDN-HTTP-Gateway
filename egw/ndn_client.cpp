...
void NdnClient::solve(std::shared_ptr<NdnMessage> message) {
    _ios.post(boost::bind(&NdnClient::solve_handler, this, message));
}

void NdnClient::solve_handler(std::shared_ptr<NdnMessage> message) {
    ndn::Interest interest(ndn::Name(message->get_name()), ndn::time::milliseconds(500));
    interest.setMustBeFresh(true);
    _face.expressInterest(interest,
                          boost::bind(&NdnClient::on_data, this, _1, _2, message),
...
void NdnClient::on_data(const ndn::Interest &interest, const ndn::Data &data, std::shared_ptr<NdnMessage> message) {
    ...
    if(data.getFinalBlockId().empty()) {
        if(!message->get_raw_stream()->is_aborted()) {
            ndn::Interest i(ndn::Name(data.getName().getPrefix(-1)).appendSegment(data.getName().get(-1).toSegment() + 1), ndn::time::milliseconds(500));
